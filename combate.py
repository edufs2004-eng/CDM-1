...
def filtrar_objetivos_validos(posibles_objetivos):
    vivos = [obj for obj in posibles_objetivos if getattr(obj, "vida_actual", 0) > 0]
    if not vivos:
        return []

    cuerpo_a_cuerpo = [obj for obj in vivos if "Ataque a distancia" not in getattr(obj, "etiquetas", [])]
...
def turno_jugador_o_aliado(atacante, enemigos_vivos, estado_combate):
    while True:
        ...
        if opcion == "1":
            ...
            habs_activas = []
            for h in getattr(atacante, "habilidades", []):
...
def iniciar_combate(jugador, aliados, enemigos, terreno="Tierra"):
    ...
    if motor_combate is not None:
        return motor_combate(
            jugador=jugador,
            aliados=aliados,
            enemigos=enemigos,
            terreno=terreno,
            turno_jugador=turno_jugador_o_aliado,
            turno_ia=turno_ia,
            mostrar_turno=lambda ronda: print(f"\n{'='*15} RONDA {ronda} {'='*15}")
        )
    ...
    while True:
        ...
        for accion in orden_turnos:
            ...
            if atacante in bando_jugador:
                turno_jugador_o_aliado(atacante, vivos_enemigos, estado)
            else:
